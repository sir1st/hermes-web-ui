...
describe('Hermes process invocation', () => {
  it('bypasses the uv hermes.exe trampoline on Windows packaged installs', async () => {
    ...
    const { execHermesFile } = await import('../../packages/server/src/services/hermes/hermes-process')

    const result = await execHermesFile(
  ...
  it('discovers sibling python.exe for a Windows hermes.exe launcher', async () => {
    ...
    try {
      ...
      await execHermesFile(join(scripts, 'hermes.exe'), ['--version'], { windowsHide: true })
  ...
  it('keeps normal command execution unchanged on non-Windows platforms', async () => {
    setPlatform('darwin')
    const { execHermesFile } = await import('../../packages/server/src/services/hermes/hermes-process')

    await execHermesFile('/opt/hermes/bin/hermes', ['--version'], { windowsHide: true })
